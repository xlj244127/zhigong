        ...
        <div class="main">
            <ul class="aul">
                ...
                <li>
                    ...
                    <input class="right" v-model="mobile" type="tel" placeholder="请输入您的联系电话（必填）"/>
                </li>
                <!--<li>-->
                    <!--<div class="left">所属行业</div>-->
                    <!--<input class="right" placeholder="建筑" readonly/>-->
                <!--</li>-->
                <li>
                    <div class="left">工种类型</div>
                    <input class="right" type="text" id="picker-device" placeholder="请选择工种类型（必填）" readonly/>
                ...
                <li>
                    ...
                    <div class="over">
                        ...
                        <input type="text" placeholder="Describe yourself" readonly id="picker-custom-toolbar_6">
                    </div>
                </li>
                <li>
                ...
                <li>
                    ...
                    <div class="otherMore">
                        <span @click="oneBtnAction()" :class="{active: littleBig==0}">大工</span>
                        <span @click="twoBtnAction()" :class="{active: littleBig==1}">小工</span>
                    </div>
                </li>
            </ul>
        </div>
        <div class="footer" @click="upAction">提交</div>
    </div>
</template>
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        props: ['codem', 'inviterId'],
        methods:{
            oneBtnAction(index){
                this.littleBig = 0;
                console.log(this.littleBig);
            },
            twoBtnAction(){
                this.littleBig = 1;
                console.log(this.littleBig);
            ...
            upAction(){
                this.code = this.codem;
                console.log(this.code);
                console.log('邀请者id',this.inviterId);
                if(this.name == ''){
                    this.$f7.alert('请输入姓名');
                    return;
                }
                ...
                wxApi.wxRegister(
                    ...
                    if(result.code==2){
                        this.$f7.alert(result.msg);
                        return;
                    }
                    if(result.data==120001){
                        this.$f7.alert(result.msg);
                        return;
                    }
                    ...
                    if(this.inviterId){
                        wxApi.addOneTimes(this.inviterId).then((res)=>{
                            console.log(res.data);
                        })
                    }
                    this.$emit('changeTate', result.data);
                })
            },
            selectType(){
...
<style scoped>
    ...
    .main .aul li .over #picker-custom-toolbar_6{
        width: 76%; height: 1.75rem; position: absolute; left: 0; top: 0; right: 0; bottom: 0; opacity: 0;
    }

    .footer{
        width: 88%; height: 1.5rem; line-height: 1.5rem; text-align: center; margin: 0 auto; background: #5A99FE; color: #fff; border-radius: 0.75rem;
        font-size: 0.8rem;  margin-bottom: 2rem; margin-top: 1rem;
    }
</style>