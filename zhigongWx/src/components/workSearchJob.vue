    ...
    <div class="title">
        <div class="left">
            <div class="top">
                ...
                <span class="t-2">获取联系方式<i>{{this.number}}</i>次</span>
            </div>
            <div class="bot">
    ...
    <div class="main">
        <ul class="aul2">
            <li v-for="item in workerList">
                <div class="a"><span>{{item.workGroup.name}}</span><img v-show="item.workGroup.user.userDetail.idNumber" src="../../static/img/yirenzheng.png"/></div>
                <div class="a2">
                ...
                <div class="a4">
                    <div class="left" @click="contactGrouper(item)" :data-id="item.id" :class="{active: item.send==true}">
                        联系
                    </div>
...
<script>
    ...
    export default {
        ...
        props: ['number', 'workerList', 'cityName', 'totalPages', 'cityId'],
        methods: {
            pickerAction(){
                this.titleHandle = 2;
            },
        ...
        mounted(){
            ...
            $$('.infinite-scroll').on('infinite', ()=>{
                ...
                setTimeout(()=>{
                    this.page++;
                    console.log('我被点击了',this.page,this.totalPages);
                    loading = false;
                    ...
                    if (this.page <= this.totalPages) {
                        wxApi.workerFindwork(
                            ...
                            this.totalItems = result.data.totalItems;
                            this.workerList = this.workerList.concat(result.data.list);
                            console.log(result.data.list);
                            for(var i=0; i<this.workerList.length; i++){
                                for(var j=0; j<this.workerTitle.length; j++){
                                    if(this.workerList[i].workType == this.workerTitle[j].code){
