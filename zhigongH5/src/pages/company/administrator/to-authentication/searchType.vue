<template>
    <div data-page="searchType" class="page navbar-fixed toolbar-fixed">
        <div class="navbar">
        ...
        <div class="page-content">
            ...
            <div class="main">
                <div class="box" v-for="item in workerKind">
                    ...
                    <div class="down">
                        <div class="type3" v-for="(one, index) in item.list" @click="selectType(one.code, index)" :data-id="one.code" :data-size="one.size" :data-index="index" :class="{ref: one.size!=3}">
                            <span>{{one.value}}</span>
                            <div class="size">
            ...
            <div class="footer" v-show="workerKind.length!=0">
    ...
    export default {
        data(){
            return{
                ...
                projectId: '',
                userId: '',
                status: ''    //项目详情添加班组长时值为(0),项目班组列表添加班组工种标签时值为(1)
            }
        },
        methods:{
            searchAction(){
            ...
            configAction(){
                        ...
                        if(this.isBlack == true){
                            projectApi.addGroupLeader(this.projectId, this.userId, this.codeList.join(',')).then((result)=>{
                                ...
                                blackListApi.addProjToBlackList(this.projectId, this.userId).then((result)=>{  //添加项目到黑名单
                                    ...
                                    this.$router.back({
                                        url: '/company/projGroupsList/'+this.projectId,
                                        force: true
                                    });
                                })
                            })
                ...
                if(this.status==2){
                    ...
                    this.$router.back({
                        pageName: 'pubilcFreeGroup',//页面的data-page值
                ...
                if(this.status==3){
                    ...
                    this.$router.back({
                        pageName: 'editResume',//页面的data-page值
                        force: true//注意此参数back方法专用
                    });
                }
            }
        ...
        mounted(){
            ...
            if(this.status==0){
                this.isBlack = this.$route.params.arguments.split('_')[0];
                this.projectId = this.$route.params.arguments.split('_')[1];
                this.userId = this.$route.params.arguments.split('_')[2];
                console.log(this.isBlack,this.projectId,this.userId);
            }
            if(this.status==1){
                this.projectId = this.$route.params.arguments.split('_')[0];
                this.userId = this.$route.params.arguments.split('_')[1];
                console.log(this.isBlack,this.projectId,this.userId);
            }
