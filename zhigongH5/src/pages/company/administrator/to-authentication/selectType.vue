    ...
    export default {
        data(){
            return{
                ...
                userId: '',
                size: '',
                status: ''    //项目详情添加班组长时值为(0),项目班组列表添加班组工种标签时值为(1),班组长发布空闲班组时值为(2),编辑简历时值为(3)
            }
        },
        methods:{
            skipAction(){
                if(this.status==0){
                    this.$router.load({url: '/company/searchType/0/'+this.isBlack+'_'+this.projectId+'_'+this.userId})
                }
                if(this.status==1){
                    this.$router.load({url: '/company/searchType/1/'+this.projectId+'_'+this.userId})
                }
                if(this.status==2){
                    this.$router.load({url: '/company/searchType/2/:arguments/'})
                }
                if(this.status==3){
                    this.$router.load({url: '/company/searchType/3/:arguments/'})
                }
            },
