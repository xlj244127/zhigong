        ...
        <div class="page-content">
            <div v-if="this.status!=0">
                <div class="head">
                    ...
                    <div class="right">{{this.title}}</div>
                </div>
                <div class="head">
                    <div class="left">工种</div>
                    <div class="right">{{this.typename}}</div>
                </div>
            </div>
            <div class="head2" v-if="this.status==0">
            ...
            <div class="main">
                <ul class="aul">
                    <li v-if="this.status==0">
                        ...
                        <div class="right" @click="addTypeAction">
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        methods:{
            ...
            addTypeAction(){
                console.log('我点击了');
                this.$router.load({url: '/company/selectTypeAddSize/2/:id/'})
            },
            reduceAction(){
            ...
            comfigPublic(){
                ...
                console.log(this.title,this.typecode,this.time,this.demandAmount,this.provinceId,this.cityId,this.districtId,this.detailAddress,this.salaryType,this.salary,this.treatmentCodes.join(','),this.tlementsCode,this.remark);
                if(this.id == 0){
                    wxApi.grouperNeedWork(
                        this.title,
                        this.typecode,
                        this.time,
                        this.demandAmount,
                        ...
                        this.groupId = result.data.id;  //保存班组id
                        this.$store.state.roleInfo.groupId = result.data.id;  //保存班组id
                        this.$router.load({url:"/user/searchWorker/"});
                    })
                }else{
                    wxApi.grouperEditNeedWork(
                        this.id,
                        this.title,
                        this.typecode,
                        this.time,
        ...
        mounted(){
            ...
            //编辑时调用
            if(this.status!=0){
                wxApi.grouperGetWorkNeedDetail(this.status).then((result)=>{
                    for(var j=0; j<this.workerTitle.length; j++){
                        if(result.data.workType == this.workerTitle[j].code){
                            result.data.workType = this.workerTitle[j].value+'_'+this.workerTitle[j].code;
                        }
                    }
                    console.log('详情',result.data);
                    this.id = result.data.id;
                    this.title = result.data.title;
                    this.typename = result.data.workType.split('_')[0];
                    this.typecode = result.data.workType.split('_')[1];
                    this.time = result.data.demandTime;
                    this.demandAmount = result.data.demandAmount;
                    this.provinceId = result.data.province.id;
                    this.cityId = result.data.city.id;
                    this.districtId = result.data.district.id;
                    $$('.ad').text(result.data.province.name);
                    $$('.ad-2').text(result.data.city.name);
                    $$('.ad-3').text(result.data.district.name);
                    this.detailAddress = result.data.address;
                    this.salaryType = result.data.salaryType;
                    this.isShow = this.salaryType;
                    if(result.data.demandType==null){
                        this.demandType = 0
                    }else{
                        this.demandType = result.data.demandType;
                    }
                    ...
                    if(this.salaryType==1){
                        if(result.data.salary.indexOf("-")==-1){
                            this.lowestSalary1 = result.data.salary
                        }else{
                            this.lowestSalary1 = result.data.salary.split('-')[0]
                            this.highestSalary1 = result.data.salary.split('-')[1]
                        }
                    }else if(this.salaryType==2){
                        if(result.data.salary.indexOf("-")==-1){
                            this.lowestSalary2 = result.data.salary
                        }else {
                            this.lowestSalary2 = result.data.salary.split('-')[0]
                            this.highestSalary2 = result.data.salary.split('-')[1]
                        }
                    }else if(this.salaryType==4){
                        if(result.data.salary.indexOf("-")==-1){
                            this.lowestSalary4 = result.data.salary
                        }else {
                            this.lowestSalary4 = result.data.salary.split('-')[0]
                            this.highestSalary4 = result.data.salary.split('-')[1]
                        }
                    }else if(this.salaryType==5){
                        if(result.data.salary.indexOf("-")==-1){
                            this.lowestSalary5 = result.data.salary
                        }else {
                            this.lowestSalary5 = result.data.salary.split('-')[0]
                            this.highestSalary5 = result.data.salary.split('-')[1]
                        }
                    }
                    $$('.y2-1').each((xls,elm)=>{
                    ...
                    this.treatmentCodes = result.data.treatments.split(',');
                    ...
                    this.tlementsCode = result.data.settlement;
            ...
            console.log($$('.demandTypeItem').length,this.demandType);
...
<style scoped>
    ...
    .head{
        width: 100%; height: 2.25rem; line-height: 2.25rem; font-size: 0.7rem; color: #666666; box-sizing: border-box; padding: 0 0.6rem; background: #fff;
        display: flex; justify-content: space-between; border-bottom: 1px solid #f4f4f4;
    }
    .head:first-of-type{
        margin-top: 0.25rem;
    }
    ...
    .main{
        width: 100%; box-sizing: border-box; background: #fff; padding: 0.25rem 1.4rem 0.7rem 0.2rem;
    }
    .main .aul{
