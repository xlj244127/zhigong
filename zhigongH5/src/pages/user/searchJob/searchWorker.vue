    ...
    <div data-page="searchWorker" class="page navbar-fixed" @page:reinit="back">
        <div class="navbar">
            <div class="navbar-inner">
                ...
                <div class="center sliding">我的招工</div>
                <div class="right sliding" @click="operationAction">管理</div>
            </div>
        </div>
        <div class="page-content">
            <div class="main" v-if="groupId!=0">
                <ul class="aul">
...
<script>
    ...
    export default {
        ...
        methods:{
            back(){
                ...
                if(this.groupId==0){
                    ...
                    this.toShare = 0;
                    wxApi.lookGrouperPublicWorkerList(this.groupId).then((result) => {
                        this.list = result.data.reverse();
                        console.log('列表', this.list);
                        for(var i=0; i<this.list.length; i++){
                            if(this.list[i].salaryType == 1){
                                this.list[i].salaryLabel= "元/天"
                            }else if(this.list[i].salaryType == 2){
                                this.list[i].salaryLabel= "元/小时"
                            }else if(this.list[i].salaryType == 4){
                                this.list[i].salaryLabel= "元/月"
                            }else if(this.list[i].salaryType == 5){
                                this.list[i].salaryLabel= "元/平方"
                            }else{
                                this.list[i].salaryLabel= "面议"
                            }
                        }
                    })
                }
            ...
            shareAction(item){
                ...
                let salary = '';
                if(item.salaryType==1){
                    salary = item.salary+'元/天';;
                }else if(item.salaryType==2){
                    salary = item.salary+'元/小时';
                }else if(item.salaryType==5){
                    salary = item.salary+'元/平方';
                }else if(item.salaryType==4){
                    salary = item.salary+'元/月';
                }else {
                    salary = '面议';
                }
                this.groupOneRecruit = {
                ...
                console.log('当个招工信息',this.groupOneRecruit);
            },
            weixinAction(){
                this.mencheng = 0;
                Wechat.share({
                    message: {
                        title: this.groupOneRecruit.title,
                        description: this.groupOneRecruit.groupName+'招'+this.groupOneRecruit.workTypeName+this.groupOneRecruit.demandAmount+'人,'+this.groupOneRecruit.salary+','+this.groupOneRecruit.treatmentsName,
                        ...
                        media: {
                            ...
                            webpageUrl: "https://open.weixin.qq.com/connect/oauth2/authorize?appid="+shareAppid+"&redirect_uri="+encodeURIComponent(shareServer)+"%2f%23%2fuser%2fshareMyRecruit%3fparams%3d"+this.userId+"_"+this.groupOneRecruit.id+"&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect"
                        },
                    },
        ...
        mounted(){
            ...
            this.groupId = this.$store.state.roleInfo.groupId;
            console.log('班组id',this.$store.state.roleInfo.groupId);
...
<style scoped>
    ...
    .main .aul li .single .down{
        width: 100%; min-height: 1.25rem; background: #F4F8FD; font-size: 0.6rem; color: #666666; box-sizing: border-box; padding: 7px 0;
    }
    ...
    .main .aul li .single .option{
        ...
        height: 1.1rem;
        padding-bottom: .1rem;
        padding-top: .1rem; line-height: 24px; text-align: right; font-size: 0.55rem; color: #999;
        overflow: hidden;
    }
    .main .aul li .single .option img{
        height: 1.1rem; display: inline-block;vertical-align: middle;
    }
