...
var userApi = {
    ...
    workGroupLeader (id) {
        return common.getRequest('/user/workGroupLeader/'+id);
    },
    //worker
    worker(id){
        return common.getRequest('/user/worker/'+id);
    },
    sendMessage (mobile) {
    ...
    //查看用户人脸照片
    facePictrue(id) {
        return common.getRequest('/user/face/'+id);
    },
    //查询登陆用户是不是班组长(不是就创建班组)
    isGrouper() {
        return common.postRequest('/workGroup/getAndSaveWorkGroup');
    },
